<x-layouts.app :title="'Echo '.str_pad($textPost->id, 3, '0', STR_PAD_LEFT).' · SOL4'">
    ...
    <div class="mt-8 grid items-start gap-12 xl:grid-cols-[minmax(0,1fr)_25rem]">
        ...
        <article class="min-w-0">
            ...
            <div class="px-1 pt-5">
                <x-post.actions :post="$textPost" type="echoes" commentsHref="#comments" />

                <x-post.tag-chips :tags="$textPost->tags" type="echoes" class="mt-5" />
            </div>
        </article>

        {{-- Comments (rendered by JS) --}}
        <x-comments.panel :endpoint="url('/echoes/'.$textPost->id.'/comments')" />
    </div>
</x-layouts.app>
